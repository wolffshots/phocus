...
func TestMesages(t *testing.T) {
	...
	t.Run("TestInterpretWriteErrors", func(t *testing.T) {
		...
		err = Interpret(client, port1, Message{uuid.New(), "QPGS1", ""}, 0*time.Second)
		assert.EqualError(t, err, "port is nil on write")

		err = Interpret(client, port1, Message{uuid.New(), "QPGS2", ""}, 0*time.Second)
		assert.EqualError(t, err, "port is nil on write")

		err = Interpret(client, port1, Message{uuid.New(), "QID", ""}, 0*time.Second)
		assert.EqualError(t, err, "port is nil on write")

		err = Interpret(client, port1, Message{uuid.New(), "SOMETHING_ELSE", ""}, 0*time.Second)
		assert.EqualError(t, err, "port is nil on write")
	})
	...
	t.Run("TestInterpretReadErrors", func(t *testing.T) {
		...
		err = Interpret(client, port2, Message{uuid.New(), "QPGS1", ""}, 0*time.Second)
		assert.EqualError(t, err, "read returned nothing")

		err = Interpret(client, port2, Message{uuid.New(), "QPGS2", ""}, 0*time.Second)
		assert.EqualError(t, err, "read returned nothing")

		err = Interpret(client, port2, Message{uuid.New(), "QID", ""}, 0*time.Second)
		assert.EqualError(t, err, "read returned nothing")

		err = Interpret(client, port2, Message{uuid.New(), "SOMETHING_ELSE", ""}, 0*time.Second)
		assert.EqualError(t, err, "read returned nothing")
	})
	...
	t.Run("TestInterpret", func(t *testing.T) {
		...
		err = Interpret(client, port1, Message{uuid.New(), "SOME_MESSAGE", ""}, 0*time.Second)
		assert.EqualError(t, err, "client not defined in send")
	})
}
